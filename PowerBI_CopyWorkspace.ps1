...
Foreach ($dashboard in $dashboards) {
    ...
    Foreach ($tile in $tiles) {
        try {
            ...
            if (!$tile.datasetId) {
                Copy-PowerBITile -WorkspaceId $source_workspace_ID -DashboardId $dashboard_id -TileId $tile_id -TargetDashboardId $target_dashboard_id -TargetWorkspaceId $target_workspace_ID
